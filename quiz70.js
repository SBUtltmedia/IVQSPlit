...
function makenewJson(title, qid, startTime, endTime) {
  ...
  fs.readFile(fileName, "utf8", function read(err, contents) {
    ...
    var currentVideo = IVQFolders+"/"+(startingDir + qid);
	...
	fs.mkdir(currentVideo+'/media', {recursive : true}, (err) => {
      if (err) {
        ...
        if (runFfmpeg){
        const ffmpeg = spawn('ffmpeg', ['-i', 'video70.mp4','-ss', `${startTimeSeconds}`, '-to', `${endTimeSeconds}`, `${currentVideo}/media/video.mp4`]);
